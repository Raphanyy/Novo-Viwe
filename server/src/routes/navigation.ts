  ...
  try {
    ...
    await client.query(
      `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, new_values, ip_address)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [userId, 'start_navigation', 'NavigationSession', session.id, JSON.stringify({
        routeId,
...
router.patch('/:id', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
    const { currentLatitude, currentLongitude, currentStopIndex, distanceTraveled } = req.body;
    ...
    const updateResult = await query(
      `UPDATE navigation_sessions 
       ...
       WHERE id = $5
       RETURNING *`,
      [currentLatitude, currentLongitude, currentStopIndex, distanceTraveled, id]
    );
  ...
  try {
    ...
    await client.query(
      `UPDATE route_stops 
       ...
       WHERE id = $4`,
      [notes || '', arrivalTime || new Date(), departureTime || new Date(), stopId]
    );
    ...
    if (remainingStops === 0) {
      ...
      const completionPercentage = Math.round((completedStops / totalStops) * 100);
    ...
    await client.query(
      `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, new_values, ip_address)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [userId, 'complete_stop', 'RouteStop', stopId, JSON.stringify({
        stopName: stop.name,
  ...
  try {
    ...
    await client.query(
      `UPDATE navigation_sessions 
       ...
       WHERE id = $6`,
      ['completed', finalNotes, actualFuelConsumption, 
       endCoordinates?.lat, endCoordinates?.lng, id]
    );
    ...
    await client.query(
      ...
      [
        session.route_id, id, userId,
        durationMinutes, session.distance_traveled || session.total_distance,
    ...
    await client.query(
      `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, new_values, ip_address)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [userId, 'complete_navigation', 'NavigationSession', id, JSON.stringify({
        durationMinutes,
    ...
    res.json({
      ...
      metrics: {
        ...
        fuelConsumption: actualFuelConsumption || session.estimated_fuel_consumption
      }
    });
