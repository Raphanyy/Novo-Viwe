...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!isStrongPassword(password)) {
      return res.status(400).json({ 
        error: 'Senha deve ter pelo menos 8 caracteres, incluindo maiúscula, minúscula, número e símbolo',
        code: 'WEAK_PASSWORD' 
      });
    }
    ...
    await query(
      ...
      [user.id, tokens.refreshToken, await bcrypt.hash(tokens.refreshToken, 10)]
    );
    ...
    await query(
      `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, ip_address, new_values)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [user.id, 'register', 'User', user.id, req.ip, JSON.stringify({ email: user.email })]
    );
...
router.post('/login', async (req, res) => {
  try {
    ...
    await query(
      ...
      [user.id, tokens.refreshToken, await bcrypt.hash(tokens.refreshToken, 10), req.ip, req.get('User-Agent')]
    );
