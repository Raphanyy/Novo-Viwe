...
export const getUserAnalyticsHandler: RequestHandler = async (req, res) => {
  try {
    ...
    const [profileResult, activitiesResult, routesResult] = await Promise.all([
      supabasePublic.from('profiles').select('*').eq('id', req.user.id).single(),
      supabasePublic.from('activities').select('*').eq('user_id', req.user.id).order('created_at', { ascending: false }).limit(10),
